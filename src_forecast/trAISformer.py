...
"""Train and evaluate the forecast variant of TrAISformer."""
...
def load_phase_data(filename):
    ...
    min_required = max(cf.min_seqlen, cf.min_past_points + cf.min_future_points)
    phase_data = [
        vessel
        for vessel in l_pred_errors
...
def build_datasets():
    ...
    for phase, filename in phase_filenames.items():
        ...
        aisdatasets[phase] = datasets.AISForecastDataset(
            data[phase],
        ...
        if getattr(cf, "log_gap_sampling", False):
            datasets.log_forecast_sampling_stats(
                aisdatasets[phase],
                phase,
    ...
    approx_future = 0.5 * (cf.min_future_points + cf.max_future_points)
    cf.final_tokens = max(1, int(len(aisdatasets["train"]) * approx_future * cf.max_epochs))
    return data, aisdatasets, aisdls


def evaluate(model, aisdls):
    future_error_sum = np.zeros(cf.max_future_points, dtype=np.float64)
    future_error_count = np.zeros(cf.max_future_points, dtype=np.float64)
    global_error_sum = 0.0
    global_future_count = 0.0
    ...
    with torch.no_grad():
        ...
        for _, batch in pbar:
            ...
            global_future_count += target_masks.sum().item()
            ...
            for idx in range(d_cpu.shape[0]):
                ...
                future_error_sum[:future_len] += errors
                future_error_count[:future_len] += 1

    mean_future_errors = np.divide(
        future_error_sum,
        np.maximum(future_error_count, 1),
        out=np.zeros_like(future_error_sum),
        where=future_error_count > 0,
    )
    overall_future_error = global_error_sum / max(global_future_count, 1.0)

    import matplotlib.pyplot as plt

    plt.figure(figsize=(9, 6), dpi=150)
    valid_steps = future_error_count > 0
    plt.plot(np.arange(1, cf.max_future_points + 1)[valid_steps], mean_future_errors[valid_steps])
    plt.xlabel("Step inside forecast horizon")
    plt.ylabel("Forecast error (km)")
    plt.title(f"Mean future forecast error = {overall_future_error:.4f} km")
    plt.xlim([1, max(2, cf.max_future_points)])
    plt.savefig(os.path.join(cf.savedir, "future_prediction_error.png"))
    plt.close()

    print(f"Mean future forecast error: {overall_future_error:.4f} km")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="TrAISformer Forecast Training")
    ...
    logging.info(
        "Forecast config: past [%d,%d], future [%d,%d], edge_case_prob=%s, "
